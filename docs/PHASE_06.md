...
Status: **implemented**. Phase 07 has **not** started.
...
Phase 07 may add TCG canonical identity and a TCG Card Central **sandbox** contract as specialized entity types / pack tables. The generic kernel already supports extra `entity_type` values and JSON attributes without schema hacks. Do not start Phase 07 until explicitly instructed.
