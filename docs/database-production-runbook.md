...
4. Point web/API `APP_DATABASE_URL` at pooled `app_user`; worker `WORKER_DATABASE_URL` at `app_worker`. Production fails closed if `APP_DATABASE_URL` is absent (do not fall back to an integration `DATABASE_URL`).
