...
Invites: an admin adds an email via Resend; the invited user signs in; membership becomes `active`. Hosted verification email uses Resend when `RESEND_API_KEY` and `RESEND_FROM_EMAIL` are configured.
...
There is no Supabase service role. Hosted web/API use `APP_DATABASE_URL` as `app_user`. Local/test may use `DATABASE_URL`. Migrations use `DATABASE_ADMIN_URL`. Server processes set transaction-local `app.current_organization_id` and `app.current_user_id` before tenant-owned queries so RLS can enforce the active tenant.
