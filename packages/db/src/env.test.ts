...
describe("migrations", () => {
  it("includes active-tenant RLS and does not RLS Better Auth identity tables", async () => {
    ...
    expect(sql).toMatch(/CREATE TABLE IF NOT EXISTS "tcg_market_feature_snapshot"/);
    expect(sql).toMatch(/CREATE TABLE IF NOT EXISTS "tcg_index_definition"/);
    expect(sql).toMatch(/CREATE TABLE IF NOT EXISTS "tcg_index_membership"/);
    expect(sql).toMatch(/CREATE TABLE IF NOT EXISTS "tcg_index_level"/);
    ...
    expect(sql).toMatch(/CREATE TABLE IF NOT EXISTS "tcg_prediction"/);
